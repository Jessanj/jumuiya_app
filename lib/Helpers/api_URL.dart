class ApiConstants {
  static String baseUrl = 'http://192.168.1.246:8000/api';
  static String testPost = '/testPost';
  static String getUsers = '/user/users';
  static String getUser = '/user/get_user/';
  static String saveUsers = '/save_member';
  static String regUsers = '/user/register';
  static String loginUser = '/user/login';
  static String addEvent = '/event/add_event';
  static String getEvent = '/event/get_events';
  static String saveGroup = '/group/create_group';
  static String getGroups = '/group/get_groups';
  static String getUserGroup = '/group/get_user_group/';
  static String joinGroup = '/add_member_to_group';
  static String searchGroup = '/group/get_group_by_number/';
  static String addAttendance = '/event/add_attendance';
  static String getAttendance = '/event/get_attendance/';
  static String getMonthEvents = '/event/get_month_events';
  static String updateUser = '/user/update_user';
  static String updateProfileImage = '/user/update_profile_image';

}